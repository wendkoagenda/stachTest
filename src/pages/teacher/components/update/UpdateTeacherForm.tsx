...
export default function UpdateTeacherForm({
  ...
  // Hook pour la mise à jour  d'un Teacher (RTK)
  const [updateTeacher, { error, isLoading }] = useUpdateTeacherMutation();
  ...
  const isFetching = fetchTeacherByIdQuery.isFetching;
  ...
  useEffect(() => {
    if (data) {
      form.reset({
        title: data?.data.teacher.title,
        banner: data?.data.teacher.banner,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    await updateTeacher(actorUpdateModel).unwrap();
